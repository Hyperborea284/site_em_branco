...
class SentimentAnalyzer:
    ...
    def set_image_paths(self, timestamp: str):
        """Define os caminhos das imagens geradas."""
        self.image_paths = {
            'pie_chart': self.generated_images_dir / f'pie_chart_{timestamp}.png',
            'bar_chart': self.generated_images_dir / f'bar_chart_{timestamp}.png',
            'line_chart': self.generated_images_dir / f'emotion_scores_{timestamp}.png'
        }
    ...
    def plot_emotion_line_charts(self, scores_list: list, paragraph_end_indices: list):
        ...
        plt.tight_layout()
        plt.savefig(self.image_paths['line_chart'])
        plt.close()

    def plot_pie_chart(self, scores_list: list):
        ...
        plt.savefig(self.image_paths['pie_chart'])
        plt.close()

    def plot_bar_chart(self, scores_list: list):
        ...
        plt.savefig(self.image_paths['bar_chart'])
        plt.close()
    ...
    def generate_html_content(self, timestamp: str, paragraphs: list, sentences: list) -> str:
        ...
        html_content = ''.join([
            ...
            f"<div style='border:1px solid black; padding:10px;'>{html_content_paragraphs}</div>",
            "<h1>Data e Hora da Análise</h1>",
            f"<div style='border:1px solid black; padding:10px;'>{timestamp}</div>",
            "<h1>Número de Parágrafos e Frases</h1>",
            f"<div style='border:1px solid black; padding:10px;'>Parágrafos: {len(paragraphs)}, Frases: {len(sentences)}</div>",
            "<h1>Gráficos Gerados</h1>",
            "<div style='border:1px solid black; padding:10px;'>",
            "<h2>Gráfico de Pizza</h2>",
            f"<img src='/app{base_path}pie_chart_{timestamp}.png' alt='Gráfico de Pizza'>",
            "<h2>Gráfico de Barras</h2>",
            f"<img src='/app{base_path}bar_chart_{timestamp}.png' alt='Gráfico de Barras'>",
            "<h2>Gráfico de Linhas</h2>",
            f"<img src='/app{base_path}emotion_scores_{timestamp}.png' alt='Gráfico de Linhas'>",
            "</div>"
        ])
        return html_content
    ...
    def execute_analysis_text(self, text: str) -> tuple:
        ...
        self.set_image_paths(timestamp)
        paragraphs, sentences = self.process_text(text)
        scores_list, paragraph_end_indices = self.analyze_paragraphs(paragraphs)
        self.plot_emotion_line_charts(scores_list, paragraph_end_indices)
        self.plot_pie_chart(scores_list)
        self.plot_bar_chart(scores_list)
